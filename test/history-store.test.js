...
const { makeEntry } = require('../lib/history.js');
...
for (let i = 2; i <= 12; i++) append(fs, file, makeEntry({ id: 'e' + i, seq: i, ts: i, prompt: 'p' }));
eq('ring capped on disk', load(fs, file).length, 10);
eq('newest survives', load(fs, file).slice(-1)[0].id, 'e12');
