// test/tabs-share.test.js â€” guards the two invariants behind "sessions are independent tabs":
//
//   1. LIVE-SHARE SURVIVAL. Sharing is a property of the PINNED tab (main's sharedTabId), not of whatever the
//      host is looking at. Every tab switch runs setActiveTab â†’ refreshSessions â†’ updateCollab â†’ ensureTunnel,
//      so keying `collabLive`/`want` on the VIEWED session made an ordinary sidebar click call shareStop() â€”
...
// pinned to the real source with grep guards â€” a refactor that changes the source shape fails the guard rather
...
// them has broken at least once because an explanatory COMMENT grew between them â€” the code was always correct.
...
function host(predicate) {
  ...
  return {
    ...
    view(sess, ws) { s.activeSession = sess; s.activeWsId = ws; ensureTunnel(); },   // setActiveTab â†’ refreshSessions â†’ updateCollab
...
}
{
  const h = host(collabLiveNew);
  h.share('sessX', 'wsA');
  h.view('sessZ', 'wsB');   // a LOCAL (non-repo) workspace â€” the old predicate's other failure mode
...
// "going liveâ€¦" presence; the full handle follows when the tunnel lands. The pin's real invariant is that
// advertise keys on the SHARED session (never the viewed one) â€” that part must never regress.
...
// It used to only PAUSE the mirror â€” and then kill the pty anyway. It now REFUSES, except for endShare (below).
...
// that tab's session can be '' â€” which makes movesShared false and would let syncShare stream the fallback project.
ok('main.js: the teardown freeze wipes the replay ring (no stale bytes for the next joiner)',
  /const freezeMirror = onPinned && endShare;\s*\n\s*if \(freezeMirror\) \{ try \{ setSharePaused\(true\); share\.resetRing\(\); share\.resetStatus\(\);/.test(MAIN));
...
ok('main.js: workspace:delete freezes AND stops advertising before the respawn (covers a web-share pinned to a session-less tab)',
  /if \(sharedHere\) \{[\s\S]{0,700}?setSharePaused\(true\); share\.resetRing\(\); share\.resetStatus\(\);[\s\S]{0,300}?stopAdvertising\(\);[\s\S]{0,300}?winSend\('share:force-end'/.test(MAIN_NC));
// â€¦and syncShare() must NOT run in that branch: every tab already points at `fallback`, so it would re-derive the
...
// Closing the shared tab is the OTHER way a live pty dies â€” respawnPty never sees it. Pausing alone left a zombie:
...
ok('main.js: closing the shared tab ends the share for real (pause + stop advertising), not just pauses it',
  /if \(sharedTabId === tabId\) \{[\s\S]{0,300}?setSharePaused\(true\); share\.resetRing\(\); share\.resetStatus\(\);[\s\S]{0,300}?stopAdvertising\(\);[\s\S]{0,300}?winSend\('share:force-end', \{ reason: 'tab-closed' \}\)/.test(MAIN));
ok('app.js: â€¦and the host is asked first (the Command Center âœ• had no confirm at all)',
...
// Occupancy is keyed by SESSION, not tab identity â€” keying it on `tabId !== activeTabId` let a background tab be
...
// collabLive is now view-independent, so the "â— Live" bar must be gated on actually VIEWING the shared tab â€”
...
// ===========================================================================================================
// 3. THE LIVE SESSION IS NOT COLLATERAL â€” the class of navigation this spec originally missed.
//
...
// guard paused the mirror (no foreign bytes leaked â€” that part worked) and then killed the conversation guests
// were watching anyway. `share.status().running` never flipped, and yet the live session was over.
//
// Only `openSession` avoided re-pointing the pinned tab. `switchWorkspace` â€” clicking a PROJECT chip, or an
// "out of sync" chip on another project's row (its handler switches workspace first) â€” went straight through.
...
const killsLiveSession = (s) => {
  ...
  if (movesShared && !endShare) return false;   // NEW: refused â†’ the pty is never killed
...
ok('respawn: â€¦and passing session:"" (workspace:open, create, adopt) is refused too',
  !killsLiveSession(Object.assign({}, shared, { session: '' })));
ok('respawn: â€¦and session:"new" (workspace:create / adopt) is refused too',
...
ok('respawn: not sharing at all â†’ ordinary respawn',
  ...
  || s.livePeers.some((p) => p.session === id));
{
  ...
  // viewing ITS project â€” see peerWsId). Keying liveness on the tab merely EXISTING hid that session's chip forever.
...
ok('delete: â€¦even when a DIFFERENT tab is the busy one',
...
{
  // â€¦and it refuses BEFORE setGenBusy: the tab keeps running its turn, so its sync gate must stay closed.
  // Scope both searches to respawnPty's body â€” `setGenBusy(tabId, false)` also appears in earlier functions.
  const body = MAIN.indexOf('function respawnPty');
  ok('main.js: â€¦and refuses BEFORE setGenBusy clears the running tab\'s sync gate',
...
// that explains it (a doc comment satisfying a guard is how a guard rots â€” it happened twice while writing this).
...
{
  // â€¦and it captures the pinned tab BEFORE clearing sharedSessionId (after, `sharedTabIdR` is still set but the
  // guard `sharedSessionId === id` is not â€” the flag would silently never be passed). Search FORWARD from the
  ...
  ok('app.js: â€¦and captures the pinned tab BEFORE clearing sharedSessionId', iCapture > -1 && iClear > iCapture);
}
ok('app.js: an ordinary session click never passes endShare',
  /claudible\.sessionOpen\(t\.tabId, id, opts && opts\.endShare\)/.test(APP));
ok('main.js: â€¦and tells the renderer to tear the tunnel down for real', /winSend\('share:force-end'/.test(MAIN));
...
// the pty never moved, so the record has to go back â€” a tab claiming a workspace its process isn't in orphans the
...
ok('app.js: switchWorkspace rolls the tab record back when main keeps the tab',
  ...
  // Deliberately NOT pinned to newBlankTab's ARGUMENT: what this check is about is the ORDER â€” the record must be
  ...
  && /if \(kept\) \{[\s\S]{0,700}?Object\.assign\(t, prev\);[\s\S]{0,600}?newBlankTab\(id,/.test(APP_NC));   // comment-stripped + widened: the kept branch gained a dedupe (and its WHY) between the restore and the open â€” the ORDER this check pins is unchanged
ok('app.js: â€¦and only resets the terminal for a switch that actually re-pointed the pty (a failed switch rolls the GLOBALS back too)',
...
ok('main.js: â€¦and a mid-turn one', /rec\.session === sid && rec\.busy\) return resolve\(\{ ok: false, error: 'busy' \}\)/.test(MAIN));
...
// C1 (HARDWARE-SMOKE-RESULTS.md): this used to be newBlankTab(newWsId, 'new') â€” which, unlike every other
...
// false there) â€” it only goes true while a share is live or the tab is mid-turn, which is the hardware condition.
...
ok('app.js: the out-of-sync chip is suppressed on a live session â€” checked in the rowâ€™s own project',
  /\} else if \(s\.diverged && !sessionIsLive\(s\.id, w && w\.id\)\) \{/.test(APP));
// The peer-hosted arm reads peersForWs(wsId || activeWsId) â€” per-row scoping with the active bucket as the
...
ok('app.js: sessionIsLive covers hosted, joined, and peer-hosted sessions â€” scoped, and ignores dead joined tabs',
...
// closeTab is reachable from the Command Center's always-visible "End this session" âœ• with zero other guard â€”
// killing a mid-turn Claude must never be silent. The confirm must sit BEFORE the actual tabClose IPC.
ok('app.js: closeTab confirms before killing a BUSY session (R1 â€” the last unguarded kill path)',
...
// down â€” this is the "I see the same live session twice" screenshot (joined row under the active project +
...
ok('app.js: a joined session renders once â€” the home tree stands down and repaints on join/leave',
...
// itself â€” `prev` is captured BEFORE activeTabId moves, and the refresh fires on live-tab transitions.
ok('app.js: switching off/onto a joined tab repaints the expanded trees (R6 â€” the vanishing joined row)',
...
// Viewing a joined tab scopes the sidebar to its HOME project (peerWsId) â€” the pinned row used to land under
...
ok('app.js: â€¦and re-points the SHARED tab last, so no abort can strand guests',
  /const ordered = owners\.filter\(\(r\) => r\.tabId !== wasSharedTab\)\.concat\(owners\.filter\(\(r\) => r\.tabId === wasSharedTab\)\);/.test(APP));
ok('app.js: â€¦ending the share only once nothing can abort',
...
// presence branch only, so while its owner browses another project the host is simply invisible to it â€” never
...
ok('joined: â€¦but NOT while its owner is browsing another project (the peer list never saw that host)',
  !autoLeaves({ peerWsId: 'wsA', liveState: 'reconnecting' }, 'wsB', true));
ok('joined: â€¦nor on a failed poll', !autoLeaves({ peerWsId: 'wsA', liveState: 'offline' }, 'wsA', false));
...
ok('app.js: â€¦and auto-leave is scoped to it',
