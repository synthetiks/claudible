...
ok('main.js: the teardown freeze wipes the replay ring (no stale bytes for the next joiner)',
  /const freezeMirror = onPinned && endShare;\s*\n\s*if \(freezeMirror\) \{ try \{ setSharePaused\(true\); share\.resetRing\(\); share\.resetStatus\(\);/.test(MAIN));   // B10: the freeze routes through the setSharePaused wrapper (same share.setPaused underneath + renderer notify)
