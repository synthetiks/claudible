...
## Security & privacy
- **Permissions are yours.** Five modes decide how much the embedded Claude Code does on its own: **Manual** (asks before every change, exactly like the normal CLI), **Accept edits** (file edits apply without asking), **Plan** (explores, never edits your source), **Auto** (routine steps run without asking, with Claude Code's own safety checks) and **Bypass permissions** (nothing is asked, every tool runs) for a one-click/voice flow. If you have never picked a mode, Claudible uses **Auto** — including on existing installs updating from an earlier release, which asked before every tool. Pick Manual if you want prompting on everything; an install that already has a mode keeps it. The mode lives on the chip in the bar across the bottom of the window. A session synced from a collaborator **never inherits your mode** — Claudible passes none for it, so it is never auto-resumed and never gets `--add-dir`; with no mode passed, Claude Code uses whatever default your own `~/.claude/settings.json` sets, which is ask-first unless you changed it.
