...
```
It prompts for an **install folder** (Enter for the default, or paste any path), auto-installs **Git for Windows** via winget if missing, clones, then `install.ps1` **refreshes itself to the latest commit** (a `git pull` on a clean checkout — a `[Update]` line you'll see; pass `-NoUpdate` to skip it), checks/installs Windows **Node 22.12+**, runs `npm install`, builds the voice services in WSL (installing their deps — may ask for your WSL sudo password), makes a Desktop shortcut, and launches. If the voice build fails (e.g. a flaky download) the installer now **warns and continues** so you still get a launchable app — retry voice later from the in-app System Check. First run downloads ~480 MB of models (10–20 min); re-run the line if interrupted. **No winget?** (older Windows) Install Git from [git-scm.com/download/win](https://git-scm.com/download/win), reopen PowerShell, then run the line.
