...
function reconcilePeerLists(fresh, prev) {
  ...
  for (const p of (Array.isArray(prev) ? prev : [])) {
    ...
    // Absent from git. Keep it ONLY when git actually reported somebody live and this entry is newer than
    // that whole read — i.e. a relay frame the branch hasn't propagated yet. An EMPTY git read is git
    // saying "nobody is live", which is authoritative: resurrecting there re-pushed ended/phase-1 rows on
    // every beacon tick (a peer's "going live…"/live row outliving the share until the 10s poll or the TTL).
    if (i === -1) { if (out.length && (p.ts || 0) > _maxTs(fresh)) out.push(p); continue; }
