...
// normalizes those inputs into the {author, machine, session-meta} shapes the event log stamps onto
// every entry — answering "who wrote this / which PC / who started the session".
...
// Session metadata captured once at session start.
function sessionMeta(o) {
  o = o || {};
  return {
    session: String(o.session || ''),
    startedBy: resolveAuthor({ username: o.startedBy }),
    startedById: o.startedById || '',
    startedAt: Number(o.startedAt) || 0,   // NOT `| 0` — would 32-bit-truncate a Date.now() ms timestamp
    machine: machineRecord(o.machine || {}),
    workspace: String(o.workspace || ''),
  };
}

module.exports = { resolveAuthor, machineRecord, sessionMeta,
  _internals: { resolveAuthor, machineRecord, sessionMeta } };
