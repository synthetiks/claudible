...
// Parse `git diff --numstat` output into the entry.files shape ([{path, add, del}]) the history feed
// summarizes ("3 files (+42/-10)"). Binary files report "-\t-\tpath" → 0/0 (counted as a changed file with
// no line stats). Pure text→data so it unit-tests without git.
function parseNumstat(text) {
  return String(text == null ? '' : text).split('\n').map((l) => l.trim()).filter(Boolean).map((l) => {
    const m = l.match(/^(\d+|-)\t(\d+|-)\t(.+)$/);
    if (!m) return null;
    return { path: m[3], add: m[1] === '-' ? 0 : (parseInt(m[1], 10) || 0), del: m[2] === '-' ? 0 : (parseInt(m[2], 10) || 0) };
  }).filter(Boolean);
}

// What changed between two checkpoints (the from-ref's tree → the to-ref's tree) as entry.files. Either id
// failing to resolve → [] (never throws): a pruned/aged-out 'from' just means "no stats", not an error.
function numstat(git, fromId, toId) {
  const a = resolve(git, fromId), b = resolve(git, toId);
  if (!a || !b) return [];
  const r = git(['diff', '--numstat', a, b]);
...
module.exports = { REF, snapshot, resolve, restore, treePaths, list, prune, numstat, parseNumstat, _internals: { REF, snapshot, resolve, restore, treePaths, list, prune, numstat, parseNumstat } };
