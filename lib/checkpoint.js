...
// The caller should snapshot the CURRENT state first (an undo point) — restore stays focused on the
// roll-back itself. Files present now but absent from the checkpoint are deleted via the injected
// rmFile(path) (relative paths); restored state is left as UNSTAGED worktree edits (index reset to HEAD).
function restore(git, id, rmFile) {
  ...
  if (git(['checkout-index', '-a', '-f']).code !== 0) return { ok: false };   // write those files into the worktree
