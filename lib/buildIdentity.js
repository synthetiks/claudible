...
// C-10.1: readGitSha's fallback for a PACKAGED install — no `.git` dir ships in the installer (package.json's
// "files" excludes it), so a packaged build had no honest commit identity at all, only the semver that stays
// frozen between releases. build.yml writes this plain-text short sha at build time, before packaging; a
// missing/malformed file (a source checkout, or a build that predates this step) degrades to null exactly
// like readGitSha does, and the caller falls back to its own default. Same {sha, short, at} shape so every
// existing BUILD.short consumer (presence stamps, the Settings About row) needs no changes.
function readPackagedBuildSha(dir) {
  try {
    const raw = fs.readFileSync(path.join(dir, 'build-sha.txt'), 'utf8').trim().toLowerCase();
    if (!/^[0-9a-f]{7,40}$/.test(raw)) return null;
    return { sha: raw, short: raw.slice(0, 12), at: 0 };
  } catch { return null; }
}

module.exports = { readGitSha, readPackagedBuildSha };
